...
import { Save, Building2, Mail, Palette, Sparkles, History, ArrowLeft, Eye, EyeOff, XCircle, Bot, Car, UserCog, ShieldCheck, Image as ImageIcon, Upload, Trash2, Layers, Key, LogIn } from 'lucide-react'
...
export function ConfiguracionPage() {
  ...
  // ----------------------------------------------------
  // Capas de Personalización (Sistema de 5 Capas de Alto Contraste)
  // ----------------------------------------------------
  ...
  async function loadConfig() {
    ...
    // Eliminar configuración de Fallback permanentemente
    ...
    if (data) {
      ...
      if (data.resend_api_key) {
        localStorage.setItem('gestarian_resend_api_key', data.resend_api_key)
      }
    }
  }
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* BOTÓN ÚNICO: CONFIGURACIÓN DE MODELOS Y CLAVES API */}
      <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950/70 to-slate-900 border-2 border-cyan-500/40 shadow-[0_0_25px_rgba(6,182,212,0.2)] flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="w-12 h-12 rounded-2xl bg-cyan-500/20 border border-cyan-400/50 flex items-center justify-center text-cyan-400 shrink-0 shadow-[0_0_15px_rgba(6,182,212,0.3)]">
            <Key className="w-6 h-6" />
          </div>
          <div>
            <h3 className="text-base font-black text-white uppercase tracking-wider">Modelos y Claves API</h3>
            <p className="text-xs text-slate-300 mt-0.5">
              Accede a la tarjeta simplificada (modelo y clave) con conexión directa a <strong>www.gestarian.com</strong> y Firebase.
            </p>
          </div>
        </div>
        <button
          ...
          className="w-full sm:w-auto px-6 py-3.5 rounded-xl bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-600 hover:from-cyan-400 hover:to-indigo-500 text-white text-xs font-black uppercase tracking-wider shadow-[0_0_20px_rgba(6,182,212,0.4)] flex items-center justify-center gap-2 shrink-0 transition-transform active:scale-95 cursor-pointer"
        >
          <Key className="w-4 h-4" />
          Configurar Modelos y Claves
        </button>
      </div>

      {/* Modal interactivo de Claves y Enlaces (Tarjeta con solo 2 líneas por modelo) */}
      <ApiKeysConfigModal
        isOpen={showApiKeysModal}
        onClose={() => setShowApiKeysModal(false)}
        onSaved={() => {
          // Confirmación silenciosa de recarga
        }}
      />
